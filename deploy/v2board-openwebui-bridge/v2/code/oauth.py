...
# 设置OAuth超时时间，默认使用AIOHTTP_CLIENT_TIMEOUT或300秒
OAUTH_TIMEOUT = AIOHTTP_CLIENT_TIMEOUT if AIOHTTP_CLIENT_TIMEOUT is not None else 300
...
class OAuthManager:
    ...
    async def handle_login(self, request, provider):
        ...
        if client is None:
            raise HTTPException(404)
        try:
            log.debug(f"Starting OAuth login for provider: {provider}")
            log.debug(f"Redirect URI: {redirect_uri}")
            log.debug(f"Client type: {type(client)}")
            log.debug(f"Client attributes: {dir(client)}")

            # 设置客户端超时配置
            client.client_kwargs['timeout'] = 30.0  # 30秒超时

            # 直接调用authorize_redirect，不设置session
            return await client.authorize_redirect(request, redirect_uri)
        except Exception as e:
            log.error(f"OAuth authorize_redirect error: {str(e)}")
            log.error(f"Error type: {type(e)}")
            log.error(f"Error details: {e.__dict__}")
            raise HTTPException(500, detail=f"OAuth authorization failed: {str(e)}")
